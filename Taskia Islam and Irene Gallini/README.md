...
## Results
To evaluate the robustness of quantum key distribution protocols under realistic conditions, we analyzed how depolarizing noise affects the quantum bit error rate (QBER), a performance metric that quantifies the proportion of incorrect bits in the shared secret key. Receiving incorrect bits can allow an eavesdropper to go undetected, so QBER reflects a QKD’s vulnerability.

We simulated the impact of increasing depolarizing noise levels on four protocols: BB84, B92, E91, and MDI-QKD. Depolarizing noise randomly alters the state of a qubit, mimicking imperfections in real world quantum communication channels. The results were visualized by plotting QBER as a function of noise probability.

Our simulations revealed that BB84 exhibited a nearly linear increase in QBER with increasing depolarizing noise. This trend is expected, as BB84 involves only two parties (Alice and Bob), and each qubit is affected independently and symmetrically by the noise. As a result, traditional error correction strategies such as redundancy based correction can be easily applied based on the estimated QBER.
...
![image alt](https://github.com/ThinkingBeyond/BeyondQuantum-2025/blob/main/Taskia%20Islam%20and%20Irene%20Gallini/QBER%20vs%20Noise%20Intensity%20-%20BB84%20with%20Depolarizing%20Noise.png?raw=true)
...
Our results show that BB84 reacts to depolarizing noise in a predictable, linear way—making it easy to apply standard error correction methods like redundancy. In contrast, MDI-QKD shows nonlinear, unpredictable QBER fluctuations due to its reliance on interference between indistinguishable photons. Noise disrupts this interference, making QBER highly sensitive and variable.

This makes MDI-QKD more difficult to stabilize under realistic conditions and highlights the need for adaptive error correction methods like Cascade, which respond to local error rates rather than relying on global averages. While MDI-QKD offers stronger security, it also demands smarter strategies to handle noise.
